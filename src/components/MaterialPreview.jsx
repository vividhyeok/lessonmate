  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-800 relative border-l border-slate-700 min-w-0">
      ...
      <div className="flex-1 min-h-0 flex flex-col bg-[#1e1e1e] overflow-hidden">
        ...
        <div className="border-t border-slate-700 bg-[#252526] p-3 space-y-3 max-h-[48%] overflow-auto custom-scrollbar">
          {currentTrack && (
            <>
              <div className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Track PPT Editor</div>
              <textarea
                value={currentTrack.pptContent || ''}
                onChange={(e) => updateTrack(currentTrack.id, 'pptContent', e.target.value)}
                placeholder="[슬라이드 제목]\n슬라이드 본문"
                className="w-full h-24 bg-[#1e1e1e] border border-slate-600 rounded px-2 py-1.5 text-sm outline-none focus:border-indigo-500 resize-none"
              />

              <div className="flex items-center gap-3">
                <div className="text-xs text-slate-400 flex items-center gap-1 shrink-0"><Type size={12} /> 글자크기</div>
                <input
                  type="range"
                  min="14"
                  max="54"
          ...
          {selectedItem ? (
            <>
              <div className="text-[11px] text-slate-400 font-bold uppercase tracking-wider flex items-center gap-2 pt-1 border-t border-slate-700">
